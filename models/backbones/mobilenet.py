...
from .constants import MC_DROPOUT_RATE, MC_STEPS
...
class MobileNetV2(nn.Module):
    ...
    def __init__(self, output_stride=8, BatchNorm=None, width_mult=1., pretrained=True, mc_dropout=False):
        ...
        if mc_dropout:  # last features, for MC train
            self.features.append(nn.Dropout2d(p=MC_DROPOUT_RATE))
        ...
        self.mid_2_features=self.features[4:8]
        self.mid_3_features=self.features[8:12]
        self.high_level_features = self.features[4:]
        self.dropout = nn.Dropout2d(p=MC_DROPOUT_RATE)  # for MC test
        self.mc_dropout = mc_dropout

    def forward(self, x):
        low_level_feat = self.low_level_features(x)
        x = self.high_level_features(low_level_feat)
        mid_2=self.mid_2_features(low_level_feat)
        mid_3=self.mid_3_features(mid_2)
        if self.mc_dropout:
            low_level_feat = self.dropout(low_level_feat)

        return x, low_level_feat,mid_2,mid_3
